  ...
  (let
    ...
    (map-set sensors
      { sensor-id: new-sensor-id }
      {
        ...
        linked-asset-id: linked-asset-id,
        owner: tx-sender
      }
    )
    (ok new-sensor-id)
...
(define-public (record-sensor-reading (sensor-id uint) (timestamp uint) (reading-value int))
  (let
    (
      (sensor (unwrap! (get-sensor sensor-id) (err u404)))
    )
    ;; Only allow authorized entities to record readings
...
(define-public (link-to-asset (sensor-id uint) (asset-id uint))
  (let
    (
      (sensor (unwrap! (get-sensor sensor-id) (err u404)))
    )
    ...
    (map-set sensors
      { sensor-id: sensor-id }
      (merge sensor { linked-asset-id: (some asset-id) })
    )
    (ok true)
  )
)
